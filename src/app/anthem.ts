...
export interface AnthemSnapshot {
  ...
  repId: string;
  t: string;          // HH:MM (Sim)
  tMin: number;       // Erzeugungs-Zeit in Minuten
  nextAtMin: number;  // angekündigte Zeit des nächsten Snapshots
  loads: number[];    // 0..1 je Segment-Index (gerundet, kompakt)
}
...
export function produceAnthemSnapshot(net: SegmentedNet, repId: string, tMin: number, norm: NormalizeParams = {}): AnthemSnapshot {
