...
export function createAcousticGuide(ctx: AudioContext): AcousticGuide {
  ...
  const thump = (t: number, f: number, g: number, dur: number) => {
    ...
    ga.gain.setValueAtTime(0, t); ga.gain.linearRampToValueAtTime(g, t + 0.006); ga.gain.linearRampToValueAtTime(0, t + dur);
    o.connect(ga).connect(master); o.start(t); o.stop(t + dur + 0.02);
    const ns = ctx.createBufferSource(); ns.buffer = noiseBuf(0.03);
    const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 900;
    const ng = ctx.createGain(); ng.gain.setValueAtTime(g * 0.6, t); ng.gain.linearRampToValueAtTime(0, t + 0.025);
    ns.connect(hp).connect(ng).connect(master); ns.start(t); ns.stop(t + 0.045);
  };
  // links = Herz („lub-dub")
  ...
  const wake = () => { if (ctx.state === 'suspended') void ctx.resume(); };
  ...
  return {
    ...
    alarm() { wake(); const t = ctx.currentTime; for (let i = 0; i < 7; i++) thump(t + i * (0.24 - i * 0.018), 95 + i * 5, 0.7, 0.13); },
  };
}
