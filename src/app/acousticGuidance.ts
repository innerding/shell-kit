...
export function createAcousticGuide(ctx: AudioContext): AcousticGuide {
  ...
  const thump = (t: number, f: number, g: number, dur: number) => {
    ...
    body.frequency.setValueAtTime(f * 1.5, t); body.frequency.exponentialRampToValueAtTime(Math.max(30, f * 0.7), t + dur * 0.5);
    const bg = ctx.createGain();
    bg.gain.setValueAtTime(0.0001, t); bg.gain.exponentialRampToValueAtTime(g, t + 0.006); bg.gain.exponentialRampToValueAtTime(0.0001, t + dur);
    body.connect(bg).connect(master); body.start(t); body.stop(t + dur + 0.03);
    const ov = ctx.createOscillator(); ov.type = 'sine';
    ov.frequency.setValueAtTime(f * 2.6, t); ov.frequency.exponentialRampToValueAtTime(Math.max(45, f * 1.2), t + dur * 0.4);
    const og = ctx.createGain();
    og.gain.setValueAtTime(0.0001, t); og.gain.exponentialRampToValueAtTime(g * 0.4, t + 0.005); og.gain.exponentialRampToValueAtTime(0.0001, t + dur * 0.6);
    ov.connect(og).connect(master); ov.start(t); ov.stop(t + dur * 0.7 + 0.03);
    ...
    const ng = ctx.createGain(); ng.gain.setValueAtTime(g * 0.5, t); ng.gain.exponentialRampToValueAtTime(0.0001, t + 0.02);
    ns.connect(hp).connect(ng).connect(master); ns.start(t); ns.stop(t + 0.04);
  };
  // links = Herz („lub-dub")
