...
function SliderStrip({ value, maxValue, onChange, expanded, onExpandChange, labels, gradient, loadLevel, labelOf }) {
    ...
    return (_jsxs("div", { style: { position: 'relative', width: expanded ? W_EXP : W_COL, height: 155, flexShrink: 0, transition: 'width 0.22s ease', userSelect: 'none', WebkitUserSelect: 'none' }, children: [_jsxs("div", { ref: trackRef, style: { position: 'absolute', left: expanded ? L_GAP_EXP : L_GAP_COL, top: 0, bottom: 0, width: STRIP_W, overflow: 'hidden', transition: 'left 0.22s ease', pointerEvents: 'none' }, children: [_jsx("div", { style: { position: 'absolute', inset: 0, borderRadius: 3, background: gradient } }), _jsx("div", { style: { position: 'absolute', inset: 1, borderRadius: 2, background: gradient } }), loadLevel != null && loadLevel < 0.999 && (
                    ...
                    _jsx("div", { style: { position: 'absolute', left: STRIP_W - 4, right: 1, top: 1, bottom: insetBottom(loadLevel), borderRadius: 1, background: 'rgba(255,255,255,0.62)' } })), maxValue < 0.99 && (_jsx("div", { style: { position: 'absolute', left: 0, right: 0, bottom: insetBottom(maxValue), height: 1, borderTop: '1px dashed rgba(255,255,255,0.4)' } })), _jsx("div", { style: { position: 'absolute', left: 0, right: 0, bottom: insetBottom(linePos), height: expanded ? 3 : 2, background: '#fff', boxShadow: '0 0 6px 1px rgba(255,255,255,0.9)', zIndex: 2 } })] }), labelOf && (() => {
                const c = labelOf(value);
                if (!c.word)
                    return null;
                return (_jsx("span", { "aria-hidden": true, style: { position: 'absolute', top: '50%', right: STRIP_W + RIGHT_GAP + SPACER + COL_W + 4, transform: 'translateY(-50%)', whiteSpace: 'nowrap', pointerEvents: 'none', opacity: expanded ? 1 : 0, transition: 'opacity 0.15s ease', background: c.color, color: readable(c.color), font: '700 11px/1 system-ui,sans-serif', letterSpacing: '0.02em', padding: '4px 9px', borderRadius: 7, boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }, children: c.word }));
...
export default function ComfortSliders({ movementValue, stayValue, stayMaxValue, onMovementChange, onStayChange, step2Active = false, scale, loadLevel, stayLoadLevel, labelOf }) {
    ...
    return (_jsxs("div", { style: { position: 'absolute', right: 0, top: 62, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, zIndex: 600 }, children: [step2Active && (_jsx(SliderStrip, { value: stayValue, maxValue: stayMaxValue, onChange: onStayChange, expanded: stayExpanded, onExpandChange: setStayExpanded, labels: { top: 'belebter', middle: 'RAST', bottom: 'ruhiger' }, gradient: gradient, loadLevel: stayLoadLevel, labelOf: labelOf })), _jsx(SliderStrip, { value: movementValue, maxValue: 1, onChange: onMovementChange, expanded: movExpanded, onExpandChange: setMovExpanded, labels: { top: 'belebter', middle: 'WEG', bottom: 'ruhiger' }, gradient: gradient, loadLevel: loadLevel, labelOf: labelOf })] }));
}
