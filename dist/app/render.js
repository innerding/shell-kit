...
// ── SVG-Helfer ──────────────────────────────────────────────────────────────
export function extractIconInner(svg) {
    return svg
        .replace(/<\?xml[^>]*\?>/g, '')
        .replace(/<!--[^>]*-->/g, '')
        .replace(/<svg[^>]*>/, '')
...
export function buildGlyphRow(deco, assets) {
    const parts = [];
    let x = 0;
    const placeGlyph = (svgRaw) => {
...
export function buildComposite(input) {
...
export function mergeOverlapping(start, swallowDist) {
    let ents = start;
    let merged = true;
    while (merged) {
        merged = false;
